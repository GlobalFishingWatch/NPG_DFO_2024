...
CREATE TEMP FUNCTION start_timestamp() AS (TIMESTAMP('2024-07-01 00:00:00 UTC'));
--
# voyages will be truncated to this end timestamp, if needed
CREATE TEMP FUNCTION end_timestamp() AS (TIMESTAMP('2024-09-06 23:59:59 UTC'));
--
------------------------------------------------------------

CREATE TABLE `world-fishing-827.scratch_joef.NPG_aiscoverage_1jul_to_6sep` AS
...
------------------------------------------------------------
  all_trips AS (
    SELECT
      ...
      ssvid,
      trip_start,
    FROM `world-fishing-827.scratch_joef.NPG_voyages_09082024`
    ),

############################################################
##### PART 2: Add AIS coverage for time period
############################################################
...
FROM(
  ...
  LEFT JOIN
    voyages_ais_hours
  USING (vessel_id, ssvid, trip_id, trip_start, trip_end)
  )
  GROUP BY 1,2,3,4,5
/*
