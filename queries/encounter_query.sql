...
voyages AS (
  ...
  FROM `scratch_joef.NPG_voyages_09082024`
  ),
...
encounters AS (
  ...
  FROM(
    SELECT
      ...
      FROM `pipe_ais_v3_published.product_events_encounter_v20240905`
      ) enc
    INNER JOIN voyages
      ON
    enc.event_start BETWEEN voyages.trip_start_timestamp AND end_day()
...
encounter_v_info AS(
  ...
  FROM(
    ...
    LEFT JOIN (
      ...
      FROM
        `pipe_ais_v3_published.product_vessel_info_summary_v20240701`) vi
      ON
