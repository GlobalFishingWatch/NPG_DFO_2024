...
-- CREATE TEMP FUNCTION  voyage_end_date() AS (TIMESTAMP('2024-06-06 23:59:59 UTC'));
CREATE TEMP FUNCTION  start_year() AS (2023);
CREATE TEMP FUNCTION  end_year() AS (2024);

## set active period of interest
CREATE TEMP FUNCTION  active_period_start() AS (TIMESTAMP('2024-06-06 00:00:00 UTC'));
CREATE TEMP FUNCTION  active_period_end() AS (TIMESTAMP('2024-06-17 23:59:59 UTC'));

## set the current day to calculate voyage duration up to (GFW) real time later in query
CREATE TEMP FUNCTION  current_day() AS (TIMESTAMP('2024-06-17 23:59:59 UTC'));
...
----------------------------------------------------------
  voyages AS (
    ...
    FROM (
      ...
      WHERE
        (trip_start >= voyage_start_date() OR trip_start IS NULL) AND trip_end IS NULL
        )
