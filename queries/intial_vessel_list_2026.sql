...
dfo_aoi as (
  SELECT
  ST_GEOGFROMTEXT("POLYGON ((146.48132506241433 39.6311253749837, 164.88965478996892 51.59045519018244, 168.26526537570766 51.11260096094804, 167.81518396427623 39.70041819005263, 160.4788569579395 25.632758928456752, 145.626170380695 25.55157401012501, 143.28574704124912 32.167929600570204, 146.48132506241433 39.6311253749837))") AS npg_2026_phase_1,
  ST_GEOGFROMTEXT("POLYGON ((143.23835549074636 32.1304243605542, 145.54668862771075 25.50754247149284, 179.51216192867042 25.64000664632826, -175.37652898234802 30.937302063149033, -174.46968382140454 48.46886008886031, 164.92019888432537 51.590418941390595, 146.4535337886566 39.602618334960596, 143.23835549074636 32.1304243605542))") AS npg_2026_phase_2,
),
...
active_vessel_ids AS (
  SELECT DISTINCT
    ...
    MAX(CAST(ST_CONTAINS((SELECT a.npg_2026_phase_1 FROM dfo_aoi), ST_GEOGPOINT(v.lon, v.lat)) AS INT64)) = 1 AS npg_2026_phase_1,
    MAX(CAST(ST_CONTAINS((SELECT a.npg_2026_phase_2 FROM dfo_aoi), ST_GEOGPOINT(v.lon, v.lat)) AS INT64)) = 1 AS npg_2026_phase_2
  FROM `global-fishing-watch.pipe_ais_v4_published.messages` v CROSS JOIN dfo_aoi a
  ...
  AND ST_CONTAINS((SELECT a.npg_2026_phase_2 FROM dfo_aoi), ST_GEOGPOINT(lon, lat))
  GROUP BY
    vessel_id),
