...
CREATE TEMP FUNCTION start_timestamp() AS (TIMESTAMP('2024-09-01 00:00:00 UTC'));
--
# voyages will be truncated to this end timestamp, if needed
CREATE TEMP FUNCTION end_timestamp() AS (TIMESTAMP('2024-11-06 23:59:59 UTC'));
--
------------------------------------------------------------

CREATE TABLE `world-fishing-827.scratch_joef.PHL_aiscoverage_sep1-nov6` AS
...
------------------------------------------------------------
  all_trips AS (
    SELECT
      ...
      ssvid,
      trip_start,
    FROM `world-fishing-827.scratch_joef.PHL_voyages`
    ),
