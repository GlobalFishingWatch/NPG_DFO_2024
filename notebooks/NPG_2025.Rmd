...
end_date <- "'2025-06-02 23:59:59 UTC'"
...
# don't run this step in the secoond iteration as we don't have updated results from IMCSN
...
```{r look at previous vessel list and current list - identify differences}
may_list <- read.csv(here::here("data","NPG_2025","NPG_vessel_list-2025-05-08.csv"), header=T)

# compare dataset length 
nrow(may_list)
nrow(dfo_voi_dataset_url)

# new vessels with setdiff
setdiff(dfo_voi_dataset_url$ssvid, may_list$ssvid)

# Only include vessels that are in the primary or secondary area of interest for the patrol
dfo_aoi <- filter(dfo_voi_dataset_url, in_primary_area_p1 | in_primary_area_p2)

# create a new vessel column for debugging  
dfo_aoi$new_ves <- ifelse(dfo_aoi$ssvid %in% setdiff(dfo_aoi$ssvid, may_list$ssvid), TRUE, FALSE)




write.csv(dfo_aoi, here::here("data","NPG_2025", paste('NPG_vessel_list','-', Sys.Date(),".csv", sep='')), row.names=F)
